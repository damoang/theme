...
// 레이아웃
if(isset($co['co_shop']) && $co['co_shop']) {
...
// 페이지 아이디
$page_id = G5_BBS_DIR.'-content-'.$co['co_id'];

if ($co['co_mobile_skin'] !== 'theme/PC-Skin' && G5_IS_MOBILE) {
...
if(is_file($skin_file)) {
    ...
    if (file_exists($himg)) { // 상단 이미지
	    ...
	    $himg_src = G5_DATA_PATH.'/content/'.$co_id.'_h';
	}
...
if ($is_admin)
    echo run_replace('content_admin_button_html', '<div class="text-center my-3"><a href="'.G5_ADMIN_URL.'/contentform.php?w=u&amp;co_id='.$co_id.'" class="btn btn-basic btn-sm"><i class="bi bi-pencil-square"></i> 내용수정</a></div>', $co);
