...
include_once(NA_PATH.'/_adm.shop.php');
...
<form name="fitemlistupdate" method="post" action="<?php echo NA_ADMIN_URL ?>/itemlistupdate.php" onsubmit="return fitemlist_submit(this);" autocomplete="off" id="fitemlistupdate">
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" rowspan="3">무배송</th>
		<th scope="col" rowspan="3">첨부</th>
    ...
    <tr class="<?php echo $bg; ?>">
        ...
        <td rowspan="3" class="td_mng td_mng_s">
            <label for="no_delivery_<?php echo $i; ?>" class="sound_only">무배송</label>
            <input type="checkbox" name="it_9[<?php echo $i; ?>]" <?php echo ($row['it_9'] == "1" ? 'checked' : ''); ?> value="1" id="it_9_<?php echo $i; ?>">
		</td>
		<td rowspan="3" class="td_mng td_mng_s">
			<?php
				$farr = sql_fetch(" select count(*) as cnt from {$g5['na_file']} where it_id = '{$row['it_id']}' ");
				$fcnt = isset($farr['cnt']) ? (int)$farr['cnt'] : '';
				echo ($fcnt) ? '<b>'.$fcnt.'</b></br>' : '';
			?>
			<a href="<?php echo NA_URL ?>/adm/attach.php?it_id=<?php echo $row['it_id']; ?>" class="win_point btn btn_03">
				<span class="sound_only"><?php echo htmlspecialchars2(cut_str($row['it_name'],250, "")); ?> 의 첨부파일 </span>등록
			</a>	
		</td>
        <td rowspan="3" class="td_mng td_mng_s">
    ...
    if ($i == 0)
        echo '<tr><td colspan="14" class="empty_table">자료가 한건도 없습니다.</td></tr>';
    ?>
    </tbody>
    </table>
</div>
※ 주의사항 : 상품여분필드 9번(it_10)은 컨텐츠/포인트/멤버십 등 결제와 동시에 완료되는 무배송 상품, 상품여분필드 10번(it_10)은 상품상세페이지 타이틀 이미지 주소 용도로 그 사용이 고정되어 있습니다.
