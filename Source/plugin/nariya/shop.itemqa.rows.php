...
$sql_common = " from `{$g5['g5_shop_item_qa_table']}` where it_id = '{$it_id}' and iq_secret = '0' $sql_where ";
...
$list = array();
$result = sql_query(" select * $sql_common order by iq_id desc limit $rows");
for ($i=0; $row=sql_fetch_array($result); $i++) {
	$list[$i] = na_iq_data($row, $thumb_w);
}
